...
define(['angular'],function(angular){
    var app = angular.module('openwebApp.psb3Controllers',[]);

    app.controller('GiftCheckCtrl',['$cookieStore','$rootScope','$scope','$state','$stateParams','$interval','$timeout','PSBCommonService',function($cookieStore,$rootScope,$scope,$state,$stateParams,$interval,$timeout,PSBCommonService){
        $scope.dateTime = {
            days:0,
            hours:0,
            minuts:0,
        ...
        if(!$rootScope.commonUtils.isEmpty($rootScope.saveCookie("psb3randomId"))){
            $scope.data.randomId = $rootScope.saveCookie("psb3randomId");
            $scope.data.mobileNo = $rootScope.saveCookie("psb3userPhoneNum");
        }

        $scope.updateMobileNo = function(){
            $scope.data.randomId = '';
            $scope.data.mobileNo = '';
            $rootScope.saveCookie("psb3randomId",null);
            $rootScope.saveCookie("psb3userPhoneNum",null);
        }


        var cookieSeconds = $cookieStore.get('psb3GiftSeconds');
        if(!$rootScope.commonUtils.isEmpty(cookieSeconds) && 0 < parseInt(cookieSeconds)){
            ...
            var cookieTimer = $interval(function(){
                if(0 == cookieSeconds){
                    ...
                    $cookieStore.remove('psb3GiftSeconds');
                }else{
                    $scope.idenCodeData.title='再次获取('+cookieSeconds + ')';
                    cookieSeconds--;
                    $cookieStore.put('psb3GiftSeconds',cookieSeconds);
                }
            },1000);
        }
            ...
            if(!$scope.idenCodeData.isGet){
                ...
                var timer = $interval(function(){
                    if(0 == seconds){
                        ...
                        seconds--;
                        $cookieStore.put('psb3GiftSeconds',seconds);
                    }
                },1000);
                ...
                PSBCommonService.sendVerifyCode($scope.idenCodeData,function(data){
                    if("success" == data.message){
                        ...
                        $scope.errorData.phoneMsg = $rootScope.errorMsg.giftCardMsg.no_allow;
        ...
        $scope.check = function(type){
            ...
            PSBCommonService.checkGift($scope.data,function(data){
                ...
                if(!$rootScope.commonUtils.isEmpty(data.resultCode) && '0' != data.resultCode){
                    $rootScope.saveCookie("psb3userPhoneNum",null);
                    $rootScope.saveCookie("psb3userPhoneNum",$scope.data.mobileNo,{expires:15});
                    $rootScope.saveCookie("psb3randomId",null);
                    $rootScope.saveCookie("psb3randomId",data.resultCode,{expires:15});
    ...
    app.controller('MarketBillCtrl',['dialog','$rootScope','$scope','$state','$stateParams','PSBCommonService',function(dialog,$rootScope,$scope,$state,$stateParams,PSBCommonService){
        ...
        $scope.dateList = [{id:'007',year:2016,month:6,selected:(6 == curDateMonth?true:false)},{id:'008',year:2016,month:7,selected:(7 == curDateMonth?true:false)},
