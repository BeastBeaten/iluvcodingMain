...
define(['angular'],function(angular){
    var app = angular.module('openwebApp.psbControllers',[]);
    ...
    app.controller('GiftCheckCtrl',['$cookieStore','dialog','$rootScope','$scope','$state','$stateParams','$interval','$timeout','PSBCommonService',function($cookieStore,dialog,$rootScope,$scope,$state,$stateParams,$interval,$timeout,PSBCommonService){
        ...
        if(!$rootScope.commonUtils.isEmpty($rootScope.saveCookie("psbrandomId"))){
            $scope.data.randomId = $rootScope.saveCookie("psbrandomId");
            $scope.data.mobileNo = $rootScope.saveCookie("psbuserPhoneNum");
        }

        $scope.updateMobileNo = function(){
            $scope.data.randomId = '';
            $scope.data.mobileNo = '';
            $rootScope.saveCookie("psbrandomId",null);
            $rootScope.saveCookie("psbuserPhoneNum",null);
        }


        var cookieSeconds = $cookieStore.get('psbGiftSeconds');
        if(!$rootScope.commonUtils.isEmpty(cookieSeconds) && 0 < parseInt(cookieSeconds)){
            ...
            var cookieTimer = $interval(function(){
                if(0 == cookieSeconds){
                    ...
                    $cookieStore.remove('psbGiftSeconds');
                }else{
                    $scope.idenCodeData.title='再次获取('+cookieSeconds + ')';
                    cookieSeconds--;
                    $cookieStore.put('psbGiftSeconds',cookieSeconds);
                }
            },1000);
        }
            ...
            if(!$scope.idenCodeData.isGet){
                ...
                var timer = $interval(function(){
                    if(0 == seconds){
                        ...
                        seconds--;
                        $cookieStore.put('psbGiftSeconds',seconds);
                    }
                },1000);
...
//                $rootScope.saveCookie("cCardNo",null);
//                $rootScope.saveCookie("cCardNo",$scope.data.cCardNo,{expires:30});
                if(!$rootScope.commonUtils.isEmpty(data.resultCode) && '0' != data.resultCode){
                    $rootScope.saveCookie("psbuserPhoneNum",null);
                    $rootScope.saveCookie("psbuserPhoneNum",$scope.data.mobileNo,{expires:15});
                    $rootScope.saveCookie("psbrandomId",null);
                    $rootScope.saveCookie("psbrandomId",data.resultCode,{expires:15});
    ...
    app.controller('MarketBillCtrl',['dialog','$rootScope','$scope','$state','$stateParams','PSBCommonService',function(dialog,$rootScope,$scope,$state,$stateParams,PSBCommonService){
        ...
        $scope.dateList = [{id:'001',year:2016,month:0,selected:(0 == curDateMonth?true:false)},{id:'002',year:2016,month:1,selected:(1 == curDateMonth?true:false)},
            ...
            {id:'009',year:2016,month:8,selected:(8 == curDateMonth?true:false)},{id:'010',year:2016,month:9,selected:(9 == curDateMonth?true:false)},
            {id:'011',year:2016,month:10,selected:(10 == curDateMonth?true:false)},{id:'012',year:2016,month:11,selected:(11 == curDateMonth?true:false)}];
    ...
    app.controller('GiftCardIndexCtrl',['$cookieStore','dialog','$rootScope','$scope','$state','$stateParams','$interval','PSBCommonService',function($cookieStore,dialog,$rootScope,$scope,$state,$stateParams,$interval,PSBCommonService){
        ...
        PSBCommonService.getGiftBill($scope.data,function(data){
            if("success" == data.message){
                ...
                if($scope.data.billCash == 100){
                    for(var i = 0; i <  $scope.cardList.length; i++){
                        if($scope.cardList[i].id == '3171492' && $scope.cardList[i].name == '天猫点券卡'){
                            $scope.cardList.splice(i,1);
                        }
                    }
                }else if($scope.data.billCash == 50){
                    var flag = false;
                    for(var i = 0; i <  $scope.cardList.length; i++){
                        if($scope.cardList[i].id == '3171492' && $scope.cardList[i].name == '天猫点券卡'){
                            flag = true;
...
//                $state.go(card.url,{data:$rootScope.commonUtils.putParams($scope.data)});
            }
            ...
            PSBCommonService.queryOrderDetail({orderNo:$scope.data.orderNo,mobileNo:$scope.data.mobileNo,randomId:$scope.data.randomId},function(data){
                if("success" == data.message){
                    ...
                    if($rootScope.commonUtils.isEmpty($scope.data.url)){
                        var i = 0;
                        if(-1 != data.data[0].cardName.indexOf('京东')){
                            i = 1;
                        }else if(-1 != data.data[0].cardName.indexOf('唯品会')){
                            i = 2;
                        }else if(-1 != data.data[0].cardName.indexOf('天猫超市')){
                            i = 6;
                        }else if(-1 != data.data[0].cardName.indexOf('天猫')){
                            i = 3;
                        }else if(-1 != data.data[0].cardName.indexOf('1号店')){
                            i = 4;
                        }else if(-1 != data.data[0].cardName.indexOf('亚马逊')){
                            i = 5;
                        }
