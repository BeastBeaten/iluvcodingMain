...
define(['angular'],function(angular){
    ...
    app.controller('GasRechargeCtrl',['$rootScope','$scope','$ionicModal','$state','$stateParams','$compile','$cookieStore','MessageService','ProductService','OrderService','PayService','MemberService','MenuService','ProductServiceForCCB',
        function($rootScope,$scope,$ionicModal,$state,$stateParams,$compile,$cookieStore,MessageService,ProductService,OrderService,PayService,MemberService,MenuService,ProductServiceForCCB){
        ...
        $scope.faceValueList = ProductServiceForCCB.getFaceValueForGasRecharge;
        ...
        $scope.checkArbitraryAmount = function (type) {
            ...
            if('1' == type){
                ...
                if(!Tools.prototype.isEmpty($scope.data.arbitraryAmount)){
                    ...
                    var flag = false;
                    if(tempCardNum >= 1 && tempCardNum <= 1000){
                        flag = true;
                    }
                    if(!flag){
                        $scope.errorData.errorMsg = MessageService.errorMsg.commonMsg.wrong_charge_amount+'，有效购买金额1000以内！';
                        $scope.errorData.popMsg = $scope.errorData.errorMsg;
                        $scope.model.show();
                        $scope.data.arbitraryAmount = '';
                    }else{
...
//                $scope.errorData.popMsg = $scope.errorData.cardMsg;
//                $scope.model.show();
//                return;
//            }
            ...
            OrderService.takeSaleOrder($scope.orderForm,function(data){
                if('success' ==  data.message){
                    ...
                    var payForm = {
                        ...
                        returnUrl:'http://web.yiqianlian.com/ccbCustom/paysuccess/'+$rootScope.code+'/'+$rootScope.menu,
                        code:$rootScope.code,
                        memberId:$rootScope.userLoginId,
                    ...
                    $scope.errorData.popMsg = $scope.errorData.orderMsg;
                    $scope.model.show();
                    $event.target.disabled = false;
                }
            });
