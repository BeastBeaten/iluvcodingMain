...
define(['angular'],function(angular){
    ...
    app.controller('FlowRechargeCtrl',['$rootScope','$scope','$state','$stateParams','$cookieStore','$compile','MessageService','ProductService','OrderService','PayService','MenuService',function($rootScope,$scope,$state,$stateParams,$cookieStore,$compile,MessageService,ProductService,OrderService,PayService,MenuService){
        ...
        $scope.chooseValue = function(flowValue){
            ...
            if(flowValue != $scope.orderForm.flowValue){
                $rootScope.initMarketInfo();
            }
            angular.forEach($scope.faceList,function(item,key){
                if(item.flowValue == flowValue){
                    if(item.stock){
                        ...
                        if($scope.data.mobileType == '移动'){
                            $scope.category = '3A';
                        }else if ($scope.data.mobileType == '联通'){
                            $scope.category = '3B';
                        }else if ($scope.data.mobileType == '电信'){
                            $scope.category = '3C';
                        }else{
                            $scope.category = '3';
                        }
                        $rootScope.getDefaultMarketBill($scope,$scope.category);
                    }else{
                        item.choosed = false;
                        $scope.orderForm.flowValue = '';
                        $scope.orderForm.perValue = '';
        ...
        $scope.useMarketBill = function(cityin){
            ...
            $rootScope.marketInfo.faceValue = $scope.orderForm.perValue;
            $state.go("market",{"category":$scope.category});
        }
...
//                        if(Tools.prototype.isEmpty($rootScope.marketInfo.billId)){
//                            $rootScope.marketInfo.faceValue = $scope.orderForm.perValue;
//                            $rootScope.getDefaultMarketBill($scope,'3');
//                        }
                    }
                });
...
//            }
            ...
            OrderService.takeSaleOrderNew($scope.orderForm,function(data){
                if('success' ==  data.message){
...
//                    }
        ...
        if(!Tools.prototype.isEmpty($rootScope.lastUrl)){
            OrderService.takeSaleOrderNew($rootScope.lastForm,function(data){
                if('success' ==  data.message){
                    $rootScope.saveCookies('flowNoes',$scope.lastForm.gameCount);
//                    if(Tools.prototype.isEmpty($scope.orderForm.memberId)){
//                        $rootScope.saveCookies('localOrder',data.data[0].billId);
//                    }
                    var payForm = {
